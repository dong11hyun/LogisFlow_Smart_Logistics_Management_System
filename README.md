...
|:----:|:----:|:-------:|:-----:|:---------:|:----------------:|
| **300만** | 13.33ms | **10.47ms** | 10.69ms | trigger | 트리거 ON, Faker 사용 |
| **300만** | 24.87ms | 23.41ms | **18.34ms** | async | 트리거 OFF, Faker 제거 |
| **3,000만** | 21.75ms | 20.40ms | **17.14ms** | async | 트리거 OFF, Faker 제거 |
| **1억** | 16.10ms | 12.45ms | **12.04ms** | async | 트리거 OFF, Faker 제거 |
...
| 조건 | sync 성능 | 원인 분석 |
|:----:|:---------:|----------|
| 트리거 ON, Faker 사용 | 13.33ms | 비정규화 컬럼이 이미 동기화된 상태 |
| 트리거 OFF, Faker 제거 | 24.87ms | 비정규화 컬럼 불일치 가능성 + 데이터 품질 차이 |
...
| 원인 | 설명 |
|------|------|
| **Lock 경합** | 동시 요청이 같은 shipment를 UPDATE할 때 대기 발생 |
| **MVCC 백그라운드 작업** | 대량 UPDATE 직후 **Autovacuum**(죽은 투플 정리)이나 **Checkpoint**(디스크 기록)가 실행되면 성능 급감 |
| **Dirty Page 해소** | 메모리(Shared Buffer)가 꽉 차서 기존 데이터를 디스크에 쓰고 비우느라 지연 발생 |

→ 프로덕션에서 **일관된 응답 시간(SLA)**이 중요하다면 trigger보다 async 계열이 안전함.
...
| 근거 | 수치 | 의미 |
|------|:----:|------|
| async vs sync (1억 건) | 12.04ms vs 16.10ms | async가 **25% 더 빠름** |
| trigger vs sync (1억 건) | 12.45ms vs 16.10ms | trigger가 **23% 더 빠름** |
| 데이터 생성 조건 차이 (300만 건) | 13.33ms vs 24.87ms | 트리거 ON이 **46% 더 빠름** |

> **Q3의 본질:** 완벽한 정답은 없다. 데이터 규모, 정합성 요구사항, 인프라 역량에 따라 적절한 트레이드오프를 선택하는 것이 핵심이다.
...
| **300만 건** | **12.48ms** | 61.61ms | PostgreSQL (80%↑) | ❌ 비활성화 | Faker 제거 |
...
```
📊 3,000만 건이 300만 건보다 더 빠른 이유?

PostgreSQL:
   300만: 24.51ms → 3,000만: 21.46ms (더 빨라짐!)
   → 데이터 생성 시 트리거 비활성화로 인덱스가 더 깨끗하게 구성됨
   → 또는 DB 캐시 워밍업 효과

Elasticsearch:
   300만: 72.51ms → 3,000만: 63.81ms (더 빨라짐!)
   → 샤드 분산이 더 효율적으로 이루어짐

📊 1억 건에서는 다시 느려짐

PostgreSQL:
   3,000만: 21.46ms → 1억: 44.79ms (109% 느려짐)
   → 파티션 프루닝에도 불구하고 데이터 증가 영향

Elasticsearch:
   3,000만: 63.81ms → 1억: 74.52ms (17% 느려짐)
   → 분산 아키텍처 덕분에 증가폭 억제
```

#### ⚠️ 핵심 결론: 규모에 따른 저장소 선택
