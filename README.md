...
> **참고:** `async_pure`, `async_fire`는 INSERT를 API에서 수행하지 않으므로 공정한 비교 대상이 아닙니다.

| 규모 | sync | trigger | async | 최적 전략 |
|:----:|:----:|:-------:|:-----:|:---------:|
| **300만** | 13.33ms | **10.47ms** | 10.69ms | trigger |
| **3,000만** | 14.62ms | **9.54ms** | 10.35ms | trigger |
| **1억** | **14.40ms** | 25.76ms | 82.24ms | sync |
...
소~중규모 (~3,000만):
   → trigger가 가장 빠름 (트리거 오버헤드 < UPDATE 직접 처리)

대규모 (1억+):
   → sync가 가장 안정적 (trigger/async 오버헤드 급증)
   → async는 INSERT 부하 + Kafka 오버헤드로 오히려 느려짐
```
...
| 전략 | 선택 기준 | 트레이드오프 |
|:----:|----------|-------------|
| **sync** | 정합성 최우선, 대규모 데이터 | 트랜잭션 시간 증가 |
| **trigger** | 소~중규모, 단순한 구조 | 숨겨진 로직, 디버깅 어려움 |
| **async** | 응답 속도 최우선, 지연 허용 | 최종 일관성, 인프라 복잡성 |
