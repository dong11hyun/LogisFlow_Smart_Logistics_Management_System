...
| 규모 | sync | trigger | async | 최적 전략 (개선율) | 트리거 | 생성기 |
|:----:|:----:|:-------:|:-----:|:------------------:|:------:|:------:|
| **300만** | 13.33ms | **10.47ms** | 10.69ms | trigger (21%↑) | ✅ 활성화 | Faker 사용 |
| **300만** | 24.87ms | 23.41ms | **18.34ms** | async (26%↑) | ❌ 비활성화 | Faker 제거 |
| **3,000만** | 21.75ms | 20.40ms | **17.14ms** | async (21%↑) | ❌ 비활성화 | Faker 제거 |
| **1억** | 16.10ms | 12.45ms | **12.04ms** | async (25%↑) | ❌ 비활성화 | Faker 제거 |
...
소~중규모 (300만 ~ 3,000만 건):
   → trigger가 가장 빠름 (8.21ms ~ 10.47ms)
   → DB 부하가 임계치를 넘지 않아 트리거의 즉시 처리가 가장 효율적

대규모 (1억 건 이상):
   → async가 가장 빠름 (12.04ms)
   → 데이터 급증 시 DB 부하 분산(Kafka) 효과가 극대화됨
```
...
| **trigger** | 중소규모 (3,000만 건 이하) | 대용량 시 DB CPU 부하 증가 가능성 |
| **async** | 대규모 (1억 건 이상) | 최종 일관성, 인프라 복잡성 |
